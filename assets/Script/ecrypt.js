var ecrypt = (function () {
	...
	var encrypt = function (data) {
		...
		if (length > 65535)
			return false;
		// console.log("Before swap",data, data.length);
	...
	var swapStr = function(str, first, last){
	    var arr = str.split('');
	    swap(arr, first, last); // Your swap function
	    return arr.join('');
	}

	var swap = function(str, first, last){
		let tmp = str[first];
		str[first] = str[last];
		str[last] = tmp;
	}
	...
	var decrypt = function (packet) {
		...
		// encrypted_data = swapStr(encrypted_data, 0 , encrypted_data.length-1);
		var data = "";
