...
cc.Class({
    ...
    calculateBallToIn(value) {
        ...
        if(value){
            ...
            numberOfBallHit =this.floatingclusters.length;
        }
        var landSlot=0;
        var conCheck =9;
        if(numberOfBallHit==3){
            landSlot=0;
        }
        else if(numberOfBallHit==4){
            landSlot=1;
        }
        else if(numberOfBallHit ==5){
            landSlot=2;
        }
        else if(numberOfBallHit==6){
            landSlot=3;
        }
        else if(numberOfBallHit==7){
            landSlot=4;
        }
        else if(numberOfBallHit==8){
            landSlot=5;
        }
        else if(numberOfBallHit==9){
            landSlot=6;
        }
        else if(numberOfBallHit>9 &&(!this.isJackPot)){
            landSlot=6;
        }
                ...
                if (this.ballcheck <= conCheck) {
                    if (payOutRange <= bigWeightage[0]) {
                        if(globalData.maxWin>=slotArray[2]){
                            tempArray.push(2);
                            globalData.maxWin-=slotArray[2];
                            repeat=false;
                        }
                    }
                    else if ((payOutRange > bigWeightage[0]) && (payOutRange <= bigWeightage[1])) {
                        if(globalData.maxWin>=slotArray[3]){
                            tempArray.push(3);
                            globalData.maxWin-=slotArray[3];
                            repeat=false;

                        }
                    }
                    else if ((payOutRange > bigWeightage[1]) && (payOutRange <= bigWeightage[2])) {
                        if(globalData.maxWin>=slotArray[1]){
                            tempArray.push(1);
                            globalData.maxWin-=slotArray[1];
                            repeat=false;

                        }
                    }
                    else if ((payOutRange > bigWeightage[2]) && (payOutRange <= bigWeightage[3])) {
                        if(globalData.maxWin>=slotArray[0]){
                            tempArray.push(0);
                            globalData.maxWin-=slotArray[0];
                            repeat=false;
                        }
                        else{
                            tempArray.push(4);
